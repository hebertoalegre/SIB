...
#########################LEER ARCHIVOS###########################
route = os.path.abspath('temporaly\consulta_25')
...
for file in tqdm(files):
    ...
    df[df.columns[0]] =df[df.columns[0]].map(bancosc)
    df['FECHA'] = fecha
    df['MONEDA'] = 'AMBAS'
    df['ORIGEN'] = 'INTEGRACION DE LA CARTERA VENCIDA'
...
print(todo)
todo.to_excel(os.path.join(os.path.abspath('outputs'),'reporte_25.xlsx'), index=False)
